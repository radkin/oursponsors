      ...
      try {
        ...
        if (firebaseUser) {
          ...
          await store.dispatch(
            createOrUpdateUser(uProfile, firebaseUser.user?.uid),
          );
        }
      } catch (error) {
        console.log(error);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.loginText}>
        <Text style={{marginHorizontal: scale(5), fontSize: scale(10)}}>
          Already Have an Account?
        </Text>
        <TouchableOpacity
          style={{marginHorizontal: scale(5)}}
          onPress={() => props.navigation.navigate('login')}>
          <Text style={{color: 'rgba(81,135,200,1)', fontSize: scale(10)}}>
            Login Here
          </Text>
        </TouchableOpacity>
      </View>
